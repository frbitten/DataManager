...
import joo.databaseManager.annotation.Table;
import joo.databaseManager.type.RelationshipInfo.TYPE;
...
public class DatabaseType {
	...
	private void addField(Class primaryClass,Field f){
		if(f.isAnnotationPresent(joo.databaseManager.annotation.Field.class)){
		...
		if(f.isAnnotationPresent(joo.databaseManager.annotation.File.class)){
		...
		if(f.isAnnotationPresent(joo.databaseManager.annotation.RelationshipOneToMany.class)){
			RelationshipInfo info=new RelationshipInfo(f, TYPE.ONE_TO_MANY);
			relationshipsOneToMany.put(info.getName().toLowerCase(), info);
		}
		if(f.isAnnotationPresent(joo.databaseManager.annotation.RelationshipManyToMany.class)){
			RelationshipInfo info=new RelationshipInfo(f, TYPE.MANY_TO_MANY);
			relationshipsManyToMany.put(info.getName().toLowerCase(), info);
		}
		if(f.isAnnotationPresent(joo.databaseManager.annotation.RelationshipOneToOne.class)){
